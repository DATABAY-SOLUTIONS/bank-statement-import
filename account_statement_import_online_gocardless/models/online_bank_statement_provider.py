...
class OnlineBankStatementProvider(models.Model):
    ...
    def _gocardless_request(
        ...
        content = {}
        url = urljoin(GOCARDLESS_API, endpoint) + "/"
    ...
    def action_select_gocardless_bank(self):
        if not self.journal_id.bank_account_id:
            raise UserError(
                self.env._("To continue configure bank account on journal %s")
    ...
    def _gocardless_select_bank_institution(self):
        ...
        if response.status_code == 400:
            raise UserError(self.env._("Incorrect country code or country not supported."))
        institutions = data
    ...
    def action_check_gocardless_agreement(self):
        ...
        redirect_url = urljoin(base_url + "/", "gocardless/response")
        _response, data = self._gocardless_request(
            "requisitions",
            request_type="post",
            data=json.dumps(
